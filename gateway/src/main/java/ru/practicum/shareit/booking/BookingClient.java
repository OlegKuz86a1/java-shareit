...
public class BookingClient extends BaseClient {
    ...
    public ResponseEntity<Object> create(long userId, BookingCreateDto bookingCreateDto) {
        if (bookingCreateDto.getStart().isAfter(bookingCreateDto.getEnd())) {
            throw new IllegalDataException("start booking must be earlier than the end");
        }
    ...
    public ResponseEntity<Object> getAllByOwner(long userId, BookingStatus state, Integer from, Integer size) {
        Map<String, Object> parameters = Map.of(
                "state", state,
                "from", from,
                "size", size
