...
public class ItemServiceImpl implements ItemService {
    ...
    public CommentDto addComment(CreateCommentDto createCommentDto) {
        Item item = itemRepository.findById(createCommentDto.getItemId()).orElseThrow(() ->
                new NotFoundException("Item not found with id: " + createCommentDto.getItemId()));
        if (bookingRepository.existsByItemIdAndStatusAndEndAfter(item.getId(), BookingStatus.APPROVED,
                dateGenerator.getCurrentTime())) {
    ...
    public ItemDto addItem(ItemCreateDto itemCreateDto) {
        ...
        System.out.println("SERVICE__ITEM = " + itemCreateDto);
        Item item = itemMapper.mapToItem(itemCreateDto);
        Item save = itemRepository.save(item);
        System.out.println("SERVICE__ITEM AFTER SAVE = " + save);
        ItemDto dto = itemMapper.toDto(save);
        System.out.println("SERVICE__ITEM AFTER SAVE AND MAPPING= " + dto);
        return dto;
    }
