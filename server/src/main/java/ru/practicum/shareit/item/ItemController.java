...
public class ItemController {
 ...
 private static final String SHARER_USER_ID_HEADER = "X-Sharer-User-Id";
    ...
    @PostMapping
    ItemDto create(@RequestHeader(SHARER_USER_ID_HEADER) Long ownerId,
                   @RequestBody ItemCreateDto itemCreateDto) {
        itemCreateDto.setOwnerId(ownerId);
        ItemDto itemDto = itemService.addItem(itemCreateDto);
        log.info("returning the item added for rent: {}", itemDto);
        return itemDto;
    }

    @PatchMapping("/{itemId}")
    ItemDto update(@RequestHeader(SHARER_USER_ID_HEADER) Long ownerId,
